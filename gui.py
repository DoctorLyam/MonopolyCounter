...
all_methods = {'Покупка участка':'buy_area', 'Заложение участка':'dep_area', 'Выкуп заложенного участка':'get_dep_area',
                ...
                'Купить вещь на аукционе':'buy_thing_in_auction'}
...
#----------------------------------
# Функция покупки участка на аукционе
def pay_area_auc():
        try:
                if chosen_method.get() == 'Купить вещь на аукционе':
                     exec(f'{chosen_user.get()}'+'.'+f'{all_methods[chosen_method.get()]}'+f'({all_things[0][chosen_area.get()]}, {entry_text.get()})')
                else:
                     print('Выберите метод "Купить вещь на аукционе"')
        except SyntaxError:
                if chosen_user.get() == 'Юзер не выбран':
                       print('Выберите игрока')
                elif len(entry_text.get()) == 0:
              ...
              print('Выберите участок')
               ...
               print('Введите цену, за которую хотите купить участок')
pay_area_btn = ttk.Button(text="Купить участок на аукционе", command=pay_area_auc).pack(anchor='sw')
#----------------------------------
# Функция покупки транспорта на аукционе
def pay_transp_auc():
        try:
                if chosen_method.get() == 'Купить вещь на аукционе':
                     exec(f'{chosen_user.get()}'+'.'+f'{all_methods[chosen_method.get()]}'+f'({all_things[1][chosen_transp.get()]}, {entry_text.get()})')
                else:
                     print('Выберите метод "Купить вещь на аукционе"')
        except SyntaxError:
                if chosen_user.get() == 'Юзер не выбран':
                       ...
                       print('Введите цену покупки транспорта')
        except KeyError:
              print('Выберите транспорт')
        except TypeError:
               print('Введите цену, за которую хотите купить транспорт')
pay_area_btn = ttk.Button(text="Купить транспорт на аукционе", command=pay_transp_auc).pack(anchor='sw')
#----------------------------------
# Функция покупки саппорта на аукционе
def pay_sup_auc():
        try:
                if chosen_method.get() == 'Купить вещь на аукционе':
                     exec(f'{chosen_user.get()}'+'.'+f'{all_methods[chosen_method.get()]}'+f'({all_things[2][chosen_sup.get()]}, {entry_text.get()})')
                else:
                     print('Выберите метод "Купить вещь на аукционе"')
        except SyntaxError:
                if chosen_user.get() == 'Юзер не выбран':
                       ...
                       print('Введите цену покупки помощника')
        except KeyError:
              print('Выберите помощника')
        except TypeError:
               print('Введите цену, за которую хотите купить помощника')
pay_area_btn = ttk.Button(text="Купить помощника на аукционе", command=pay_sup_auc).pack(anchor='sw')
